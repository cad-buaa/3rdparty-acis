...
// This class implements a variable-length list for entity_handle objects.  The list
...
//
// The entity_handle_list class is a wrapper for the entity_handle_list_eng class, 
// which implements the core functionality.

#ifndef ENTITY_HANDLE_LIST_HXX
#define ENTITY_HANDLE_LIST_HXX
 ...
 * <br>
 * <b>Role:</b> This class is a list object for <tt>entity_handle</tt> objects that is intended to have the look and feel
 ...
 * <br>
 * The holding and counting mechanisms are specified when the <tt>entity_handle_list</tt> object is constructed.
 * Additionally, the holding state can be changed using <tt>entity_handle_list::set_hold_state</tt>.
 * <br><br>
 * Handles are added to the end of the list using <tt>entity_handle_list::add</tt> which ensures that the handles are not
 * already in the list (i.e. set behavior).  The <tt>entity_handle_list::remove</tt> methods take entries off the list, 
 * but a tombstone remains at the position of the former entries.  The <tt>entity_handle_list::add_notify</tt> and 
 * <tt>entity_handle_list::remove_notify</tt> methods notify the caller when the list is actually modified.
 * <br>
 * The position of current entries can be determined using <tt>entity_handle_list::lookup</tt>, and the entry at a specified position
 * may be accessed using the overloaded <tt>"[ ]"</tt> index operator.  The number of entries in the list is returned by
 * <tt>entity_handle_list::count</tt> (includes tombstones) and <tt>entity_handle_list::iteration_count</tt> (excludes tombstones).
 * The preferred way of accessing the data items is through <tt>entity_handle_list::init</tt> which rewinds the list
 * and <tt>entity_handle_list::next</tt> which returns the next undeleted item.  The <tt>entity_handle_list::first</tt> method
 * rewinds the list and returns the first non-deleted entry.
 * <br><br>
 * The current implementation uses hashing so that <tt>entity_handle_list::lookup</tt> is fast;
 * it is also efficient for very short lists and for repeated lookups of the same entity handle.
 * <br><br>
 * When a group of entity handles must be returned, and the number of arguments is not known in
 * advance, the system returns the arguments as an <tt>entity_handle_list</tt>. For instance, when the routine
 * <tt>api_asm_get_model_refs</tt> is used to find every immediate model_ref in an assembly model, the handles found
 * are put into an <tt>entity_handle_list</tt> that is returned.
 * <br>
 * <pre>
 * entity_handle_list my_handles;
 * api_asm_get_model_refs( assembly_model, my_handles );
 * for ( entity_handle* handle = my_handles.first(); handle != NULL; handle = my_handles.next() )
 * {
 * .
 * .
 * .
 ...
 * </pre>
 * <br>
 * <b>Note: </b>The general method for retrieving the number of entries in an <tt>entity_handle_list</tt> is iteration_count().
 ...
 * <br>
 * If the count method is used to determine the number of entries in an <tt>entity_handle_list</tt> (including tombstones),
 ...
 * <br><br>
 * The <tt>entity_handle_list</tt> class is a variable length associative array of <tt>entity_handle</tt> pointers. When using
 * the subscript operator, a cast is required to change the <tt>entity_handle</tt> pointer into the correct type.
 * Many ACIS internal algorithms use <tt>entity_handle_list</tt> including the save, and restore algorithms.
 * <tt>entity_handle_list</tt> is also useful in <tt>ACIS</tt> components and applications.
 * <br><br>
 * @see entity_handle, asm_model
 */
class DECL_ASM entity_handle_list : public ACIS_OBJECT {

	entity_handle_list_eng* list_ptr;		// list engine
	entity_handle_list_eng* get_list();		// for internal use, instantiates the engine if not already present
...
public:
 ...
 /**
  * Default constructor, creating an <tt>entity_handle_list</tt>.
  * <br><br>
 ...
 /**
  * C++ constructor, creating an <tt>entity_handle_list</tt> with the specified holding behavior.
  * <br><br>
  ...
  * @see asm_hold_state
  */
	entity_handle_list( asm_hold_state state );

 /**
  * C++ constructor, creating an <tt>entity_handle_list</tt> with the behavior specified by the <tt>asm_list_options</tt> object.
  * <br><br>
  * @param list_opts
  ...
  */
	entity_handle_list( asm_list_options* list_opts );

 /**
  * C++ destructor, deleting an <tt>entity_handle_list</tt>.
  * <br><br>
 ...
 /**
  * C++ constructor, creating an <tt>entity_handle_list</tt> from an array with the behavior specified by the <tt>asm_list_options</tt> object. 
  * <br><br>
  ...
  * @see asm_list_options
  */
	entity_handle_list( int count, entity_handle** handle_array, asm_list_options* list_opts = NULL, logical check = TRUE );
  ...
  * list to copy.
  */
	entity_handle_list( const entity_handle_list& list_copy);
  ...
  * <br><br>
  * @param entity_handle_list
  * entity_handle list.
  */
	entity_handle_list& operator=( const entity_handle_list& entity_handle_list);
  ...
  * <br><br>
  * @param handle
  * entity_handle to add.
  * @param check
  * check unique.
  */
	int add( entity_handle* handle, logical check = TRUE );

 /**
  * Adds an entity handle to the list, returns its index number and notifies if the handle is new.
  * <br><br>
  * <b>Role:</b>Same as <tt>entity_handle_list::add</tt>, but notifies the caller if the handle is added
  ...
  */
	int add_notify( entity_handle* handle, logical& was_added, int& entry_count );

 /**
  * Adds the handles of an <tt>entity_handle_list</tt> to this <tt>entity_handle_list</tt>.
  * <br><br>
  * <b>Role:</b>Same as <tt>entity_handle_list::add</tt> for all handles in the input list.
  * <br><br>
  * @param entity_handle_list
  * entity_handle list.
  * @param check
  * check unique.
  */
	void add( const entity_handle_list& handle_list, logical check = TRUE );
  ...
  * entry to lookup
  */
	int lookup( const entity_handle* handle ) const;
  ...
  * entry to remove
  */
	int remove( const entity_handle* handle );

 /**
  * Remove the specified entry from the list and return its index.
  * <br><br>
  * <b>Role:</b> Same as <tt>entity_handle_list::remove</tt>, but the caller is notified if the entry is actually deleted
  ...
  * number of adds minus removes
  */
	int remove_notify( const entity_handle* handle, logical& was_removed, int& entry_count );

 /**
  * Removes asm handles in the given list from the list; however, it does not free space.
  * <br><br>
  * <b>Role:</b> Same as <tt>entity_handle_list::remove</tt> for a list of handles.
  * <br><br>
  * @param handle_list
  * entity_handle list.
  */
	void remove( const entity_handle_list& handle_list );

 /**
  * Remove the entry at the specified index from the list and return its index.
  * <br><br>
  * <b>Role:</b> Same as <tt>entity_handle_list::remove</tt>.
  * <br><br>
  * @param index
  ...
  * entity handle.
  */
	int entry_count( const entity_handle* list_member );
...
/**
  ...
  * or the constant LIST_ENTRY_DELETED if the indexed entity handle was removed from the list.
  * <br><br>
  * @param index
  * integer.
  */
	entity_handle *operator[]( int index ) const;
  ...
  */
	entity_handle *first() const;

 /**
  * Returns the next undeleted (live) entry.
  */
	entity_handle *next() const;
  ...
  * integer.
  */
	entity_handle *next_from( int& index );
  ...
  * <br><br>
  * <b>Role:</b>  It does not include the size of the individual <tt>entity_handles</tt> themselves.
  * <br><br>
  * @param countSelf
  * count self or not.
  */
