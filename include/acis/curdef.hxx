...
class DECL_KERN curve_boundcyl : public ACIS_OBJECT {
  ...
  public:
    ...
    curve_boundcyl(SPAposition const& root, SPAunit_vector const& axis, double rad): _root_point(root), _axis(axis), _radius(rad) {}
...
class DECL_KERN curve_tancone : public ACIS_OBJECT {
  public:
    ...
    curve_tancone(SPAunit_vector const& ax, double ang, logical app, logical over): axis(ax) {
        angle = ang;
        approx = app;
        oversize = over;
    }
...
enum evaluate_curve_side { evaluate_curve_above, evaluate_curve_below, evaluate_curve_unknown };
...
class DECL_KERN curve : public ACIS_OBJECT {
  ...
  public:
     ...
     */
    logical operator!=(const curve& cur) const { return !(*this == cur); }
    ...
    */
     ...
     * transformation.
     */
    SPAbox bound(double start, double end, const SPAtransf& transf = *(SPAtransf*)NULL_REF) const { return bound(SPAinterval(start, end), transf); }
     ...
     * weak flag.
     */
    virtual void point_perp(const SPAposition& point, SPAposition& foot, SPAunit_vector& tangent, SPAvector& curvature, const SPAparameter& param_guess = *(SPAparameter*)NULL_REF, SPAparameter& actual_param = *(SPAparameter*)NULL_REF,
                            logical f_weak = FALSE) const = 0;
    /**
     ...
     */
    virtual int evaluate(double param,               // Parameter
     ...
     * the evaluation location - above, below, or "don't care".
     */
    virtual int evaluate_iter(double param,               // Parameter
...
#    define ALL_CURVE_DERIVATIVES 9999
     ...
     */
    virtual int accurate_derivs(const SPAinterval& range = *(SPAinterval*)NULL_REF
  ...
  protected:
     ...
     * the evaluation location - above, below, or "don't care".
     */
    virtual int finite_difference_derivatives(double param,             // Parameter
                                              SPAposition& pos,         // Point on curve at given SPAparameter
     ...
     * @nodoc
     */
    virtual void step_from_guess(const SPAposition& pos, double& param_wanted) const;

    /**
     * @nodoc
     */
    virtual void point_perp_internal(const SPAposition& point, double param_wanted, double& param_best, SPAposition& foot_best, SPAunit_vector& tan_best, SPAvector& foot_dt_best, SPAvector& foot_ddt_best, logical linear_search, double tol,
                                     logical& point_found) const;
  ...
  public:
     ...
     * If approx_ok is true, and the curve has a B-spline approximation, then this approximation is used in the length calculation.
     */
    virtual double length_param(double param,   // datum parameter
                                double length,  // arc length
                                logical approx_ok = TRUE) const = 0;
     ...
     * actual parameter.
     */
    logical test_point(const SPAposition& pos, const SPAparameter& param_guess = *(SPAparameter*)NULL_REF, SPAparameter& param_actual = *(SPAparameter*)NULL_REF) const { return test_point_tol(pos, 0, param_guess, param_actual); }
...
#    if defined D3_STANDALONE || defined D3_DEBUG
    ...
    */
     ...
     * @nodoc
     */
    friend DECL_KERN D3_istream& operator>>(D3_istream&, curve*&);
    /*
    // tbrv
    */
    /**
     * @nodoc
     */
    friend DECL_KERN D3_ostream& operator<<(D3_ostream&, const curve&);
 ...
 * @nodoc
 */
enum extremum_type { extremum_unknown, extremum_minimum, extremum_maximum };
...
class DECL_KERN restore_cu_def : public ACIS_OBJECT {
  ...
  public:
    restore_cu_def(int,  // internal type number (for old
                         // save files)
