...
class DECL_KERN pcurve : public ACIS_OBJECT {
  ...
  public:
     ...
     * owns bs2.
     */
    pcurve(bs2_curve curve, double para, surface const& sur, int knot = -1, int hull_cur = -1, int hull_angle = -1, int hull_selfintr = -1, logical owns_bs2 = TRUE, double = -1.0);
     ...
     * integer.
     */
    pcurve(intcurve const& cur, int integer);
     ...
     * parameter.
     */
    pcurve(curve const& cur, surface const& sur, double para);
     ...
     * underlying curve.
     */
    pcurve(law* law_type, double start_param, double end_param, surface const& sur, double tol = SPAresabs, bs2_curve cur = NULL);
     ...
     * curve.
     */
    pcurve(logical udir, double param, surface const& sur, curve const& cur = *(curve*)NULL_REF);
    ...
    */
     ...
     * knots.
     */
    void set_checked_range(const SPAinterval& new_range, int num_knots = 0, double* knots = NULL);
    /**
     ...
     * derivative.
     */
    int add_bs2_knot(double new_knot_param, int mult, SPApar_pos& new_knot_uv = *(SPApar_pos*)NULL_REF, SPApar_vec& new_knot_deriv_below = *(SPApar_vec*)NULL_REF, SPApar_vec& new_knot_deriv_above = *(SPApar_vec*)NULL_REF);
    ...
    */
     ...
     * @nodoc
     */
    friend DECL_KERN pcurve operator+(SPApar_vec const& pv, pcurve const& pc) { return pc + pv; }
    /**
     ...
     * second parameter.
     */
    void reparam(double firstp, double secondp);
    ...
    */
     ...
     * parameter position.
     */
    pcurve* split(double par, SPApar_pos const& par_pos = *(SPApar_pos*)NULL_REF, SPApar_vec const& par_pos_vec = *(SPApar_vec*)NULL_REF);
     ...
     * slope.
     */
    SPAinterval trim(const SPAinterval& new_range, const SPApar_pos& start_uv = *(SPApar_pos*)NULL_REF, const SPApar_vec& start_duv = *(SPApar_vec*)NULL_REF, const SPApar_pos& end_uv = *(SPApar_pos*)NULL_REF,
     ...
     * curve.
     */
    SPApar_box bound(SPAinterval const& int_name, const curve* true_cu = NULL) const;
     ...
     * curve.
     */
    SPApar_box bound(double first_param, double second_param, const curve* true_cu = NULL) const;
     ...
     * knots.
     */
    void add_knots_at_discontinuities(const curve* cu, int num_knots = 0, double* knots = NULL);
     ...
     */
    void validity_checks(const curve* cu, const SPAinterval& chkd_range, logical& knots_on_true_cur, logical& knot_tangents_ok, logical& hull_turn_angles_ok, logical& hull_intersects, logical& hull_contains_true_cu);
     ...
     * knots.
     */
    logical check_knots_on_true_curve(const curve* true_cu, BOUNDED_SURFACE* bsf, int deg, int& num_knots, double*& knots);
     ...
     * knots.
     */
    logical check_knot_tangents(const curve* true_cu, BOUNDED_SURFACE* bsf = NULL, int num_knots = 0, double* knots = NULL);
     ...
     * control points.
     */
    logical check_hull_turning_angle(const curve* true_cu, BOUNDED_SURFACE* bsf, int deg, int& num_knots, double*& knots, int& num_ctrlpts, SPApar_pos*& ctrlpts);
     ...
     * control points.
     */
    logical check_hull_intersection(int deg = 0, int num_knots = 0, double* knots = NULL, int num_ctrlpts = 0, SPApar_pos* ctrlpts = NULL);
     ...
     * control points.
     */
    logical check_hull_curve_enclosure(const curve* true_cu, BOUNDED_SURFACE* bsf = NULL, int deg = 0, int num_knots = 0, double* knots = NULL, int num_ctrlpts = 0, SPApar_pos* ctrlpts = NULL);
    ...
    */
     ...
     * file name.
     */
    void debug(char const* title, FILE* file_name = debug_file_ptr) const;
...
class DECL_KERN par_cur : public subtrans_object {
  ...
  protected:
    ...
    SPAinterval checked_range() const { return check_range; }
    int bezier_form() {
        if(_bezier_form == -1) check_bezier_form();
        return _bezier_form;
    }
    int knots_on_true_curve() const { return knots_on_true_cu; }
    int accurate_knot_tangents() const { return acc_knot_tangents; }
    int hull_turning_angles_ok() const { return hull_angles_ok; }
    int hull_self_intersects() const { return hull_self_ints; }
    double hull_distance() const { return hull_dist; }
    ...
    logical check_bezier_form();
    logical make_bezier_form();
    void set_knots_on_true_curve(int knots_on_cu) { knots_on_true_cu = knots_on_cu; }
    void set_accurate_knot_tangents(int acc_tangents) { acc_knot_tangents = acc_tangents; }
    void set_hull_turning_angles_ok(int angles_ok) { hull_angles_ok = angles_ok; }
    void set_hull_self_intersects(int self_ints) { hull_self_ints = self_ints; }
    void set_hull_distance(double dist);
    ...
    virtual int add_bs2_knot(double new_knot_param, int mult, SPApar_pos& new_knot_uv = *(SPApar_pos*)NULL_REF, SPApar_vec& new_knot_deriv_below = *(SPApar_vec*)NULL_REF, SPApar_vec& new_knot_deriv_above = *(SPApar_vec*)NULL_REF);
    ...
    virtual void reparam(double, double) = 0;
    ...
    virtual void split(double, SPApar_pos const&, par_cur* [2], SPApar_vec const& = *(SPApar_vec*)NULL_REF) = 0;
    ...
    logical check_knots_on_true_curve(const curve* true_cu, logical rev, SPApar_vec offset, BOUNDED_SURFACE* bsf, int deg, int& num_knots, double*& knots);
    ...
    logical check_knot_tangents(const curve* true_cu, logical rev, BOUNDED_SURFACE* bsf = NULL, int num_knots = 0, double* knots = NULL);
    ...
    logical check_hull_turning_angle(const curve* true_cu, logical rev, SPApar_vec offset, BOUNDED_SURFACE* bsf, int deg, int& num_knots, double*& knots, int& num_ctrlpts, SPApar_pos*& ctrlpts);
    ...
    logical correct_hull_turning_angle(const curve* true_cu, SPApar_vec offset, BOUNDED_SURFACE*& bsf, int deg, int& num_knots, double*& knots, int& num_ctrlpts, SPApar_pos*& ctrlpts, int& knot_index, int& check_n_knots, int& ctrlpt_index, double knottol);
    ...
    logical iter_check_hull_turning_angle(const curve* true_cu, SPApar_vec offset, BOUNDED_SURFACE*& bsf, int deg, int& num_knots, double*& knots, int& num_ctrlpts, SPApar_pos*& ctrlpts, int& knot_index, int& check_n_knots, int& ctrlpt_index,
                                          SPApar_dir& prev_dir, SPApar_dir& curr_dir, double knottol, logical& start_over);
    ...
    logical check_hull_curve_enclosure(const curve* true_cu, logical rev, SPApar_vec offset, BOUNDED_SURFACE* bsf = NULL, int deg = 0, int num_knots = 0, double* knots = NULL, int num_ctrlpts = 0, SPApar_pos* ctrlpts = NULL);
