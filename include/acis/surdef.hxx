 ...
 */
enum evaluate_surface_quadrant { evaluate_surface_above_above, evaluate_surface_below_above, evaluate_surface_below_below, evaluate_surface_above_below, evaluate_surface_unknown };
 ...
 */
enum sweep_path_type { unknown_path_type, straight_path_type, circular_path_type, defined_path_type };
// STI swa END
...
class DECL_KERN surface : public ACIS_OBJECT {
  ...
  public:
     ...
     */
    logical operator!=(const surface& surf) const { return !(*this == surf); }
    /*
    // tbrv
    */
    /**
     ...
     * transformation to apply.
     */
    virtual surf_normcone normal_cone(const SPApar_box& parbox,  // SPAparameter bounds
                                      logical approx_OK = FALSE, const SPAtransf& transf = *(SPAtransf*)NULL_REF) const;
     ...
     * parameter guess.
     */
    virtual void point_prin_curv(const SPAposition& pos,
                                 ...
                                 const SPApar_pos& param_guess = *(SPApar_pos*)NULL_REF) const = 0;
     ...
     */
    virtual double point_cross(const SPAposition& pos, const SPAunit_vector& normal, const SPApar_pos& param_guess, logical use_sided_eval, logical right_side) const;
    ...
    void point_perp(const SPAposition& pos, SPAposition& foot, SPAunit_vector& normal, const SPApar_pos& param_guess = *(SPApar_pos*)NULL_REF, SPApar_pos& param_actual = *(SPApar_pos*)NULL_REF, logical f_weak = FALSE) const {
        point_perp(pos, foot, normal, *(surf_princurv*)NULL_REF, param_guess, param_actual, f_weak);
    }
    /**
     ...
     */
    virtual void eval(const SPApar_pos& param, SPAposition& pos,
     ...
     * curvature along the second axis.
     */
    virtual void eval_prin_curv(const SPApar_pos& pos,
                                SPAunit_vector& axis1,  // first axis direction
                                double& cur1,           // curvature in first direction
                                SPAunit_vector& axis2,  // second axis direction
                                double& cur2            // curvature in second direction
     ...
     */
    virtual int evaluate(const SPApar_pos& param,    // Parameter
     ...
     * the evaluation location - above, below, or "don't care" (for each parameter direction).
     */
    virtual int evaluate_iter(const SPApar_pos& param,    // Parameter
...
#    define ALL_SURFACE_DERIVATIVES 9999
     ...
     */
    virtual int accurate_derivs(const SPApar_box& box = *(SPApar_box*)NULL_REF
  ...
  protected:
     ...
     * the evaluation location - above, below, or "don't care" (for each parameter direction).
     */
    int finite_difference_derivatives(const SPApar_pos& param,  // Parameter
                                      SPAposition& pos,         // Point on surface at given SPAparameter
  ...
  public:
     ...
     */
    virtual logical singular_u(double uparam  // constant u SPAparameter
    ) const;
    /**
     ...
     */
    virtual logical singular_v(double vparam  // constant v SPAparameter
    ) const;
     ...
     */
    virtual curve* u_param_line(double vparam  // constant v SPAparameter
    ) const;
    /**
     ...
     */
    virtual curve* v_param_line(double uparam  // constant u SPAparameter
    ) const;
     ...
     * actual parameter.
     */
    logical test_point(const SPAposition& pos, const SPApar_pos& uv_guess = *(SPApar_pos*)NULL_REF, SPApar_pos& uv_actual = *(SPApar_pos*)NULL_REF) const { return test_point_tol(pos, 0, uv_guess, uv_actual); }
     ...
     * actual parameter.
     */
    virtual logical test_point_tol(const SPAposition& pos,
                                   double tol = 0,  // defaults to SPAresabs
                                   const SPApar_pos& uv_guess = *(SPApar_pos*)NULL_REF, SPApar_pos& uv_actual = *(SPApar_pos*)NULL_REF) const = 0;
...
#    if defined D3_STANDALONE || defined D3_DEBUG
     ...
     * @nodoc
     */
    friend DECL_KERN D3_istream& operator>>(D3_istream&, surface*&);
    /**
     * @nodoc
     */
    friend DECL_KERN D3_ostream& operator<<(D3_ostream&, surface const&);
    /**
     * @nodoc
     */
...
#    endif
  ...
  protected:
     ...
     * @nodoc
     */
    virtual logical validate_uv_guess(const SPApar_pos& uv_guess, SPApar_pos& valid_uv_guess) const;
    /**
     * @nodoc
     */
 ...
 * @nodoc
 */
enum surface_vardir { surface_var_unknown, surface_var_ugreater, surface_var_vgreater };
...
class DECL_KERN surf_normcone : public ACIS_OBJECT {
  public:
     ...
     * inside or outside?
     */
    surf_normcone(const SPAunit_vector& ax, double ang, surface_vardir vdir, logical approx_OK, logical over);
 ...
 * @nodoc
 */
inline surf_normcone::surf_normcone(const SPAunit_vector& ax, double ang, surface_vardir vdir, logical app, logical over): axis(ax) {
    angle = ang;
    vardir = vdir;
    approx = app;
    oversize = over;
 ...
 * @nodoc
 */
inline surf_princurv::surf_princurv(const SPAunit_vector& d1, double c1, const SPAunit_vector& d2, double c2): dir1(d1), dir2(d2) {
    curv1 = c1;
    curv2 = c2;
}
...
class DECL_KERN restore_su_def : public ACIS_OBJECT {
  ...
  public:
    restore_su_def(int,          // internal type number (for old
                                 // save files)
 ...
 * @nodoc
 */
DECL_KERN void surf_deriv_to_curv(SPAvector const*,  // array of two first derivatives
