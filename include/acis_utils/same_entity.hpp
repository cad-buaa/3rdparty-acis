 ...
 * 若不为空则返回entity1中导致匹配失败的entity，可能为entity1自身或者entity1直接下属的某个entity
 */
bool same_coedge(const COEDGE *coedge1, const COEDGE *coedge2, bool check_topology = true, bool check_direction = true, ENTITY_LIST *ill_entity_list = nullptr, ENTITY_LIST *failed_entity_list = nullptr)
{
...
inline bool same_bs3_curve(const bs3_curve cur1, const bs3_curve cur2, double tol = SPAresabs)
{
    ...
    if (rat1 != rat2)
        return false;
    if (nums_ctrlpts1 != nums_ctrlpts2)
        return false;
    for (int i = 0; i < nums_ctrlpts1; i++)
    {
        if (!same_point(ctrlpts1[i], ctrlpts2[i]))
            return false;
        if (rat1 == true)
        {
            if (abs(weights1[i] - weights2[i]) > tol)
                return false;
        }
    }
    if (num_knots1 != num_knots2)
        return false;
    for (int i = 0; i < num_knots1; i++)
    {
        if (abs(knots1[i] - knots2[i]) > tol)
            return false;
    }
...
inline bool same_curve_surf_int(const curve_surf_int *gme_inters, const curve_surf_int *acis_inters, double tol = SPAresabs)
{
    ...
    while (gme_inters && acis_inters)
    {
        ...
        if (fabs(gme_int.x() - acis_int.x()) < SPAresabs && fabs(gme_int.y() - acis_int.y()) < SPAresabs && fabs(gme_int.z() - acis_int.z()) < SPAresabs && fabs(gme_inters->param - acis_inters->param) < SPAresabs &&
            fabs(gme_inters->surf_param.v - acis_inters->surf_param.v) < SPAresabs && fabs(gme_inters->surf_param.u - acis_inters->surf_param.u) < SPAresabs)
        { // 比较gme和acis的每个交点
